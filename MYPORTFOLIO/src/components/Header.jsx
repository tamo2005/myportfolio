    ...
    <>
      <header 
        className={`fixed top-0 w-full z-50 transition-all duration-500 ${
          isScrolled 
            ? 'bg-white/10 backdrop-blur-xl shadow-2xl border-b border-white/20 dark:bg-[#0D0D0D] dark:bg-opacity-80 dark:border-white/5' // Dark mode colors for header background
            : 'bg-transparent'
      ...
      >
        <nav className="container mx-auto px-6 py-4">
          <div className="flex justify-between items-center">
                ...
                return (
                  <button
                    ...
                    className={`relative group flex items-center space-x-2 px-4 py-2 rounded-full transition-all duration-300 ${
                      isActive 
                        ? 'bg-gradient-to-r from-orange-500 to-orange-600 text-white shadow-lg dark:from-[#FF4C1B] dark:to-[#FF3C00] dark:text-[#F5F5F5]' // Dark mode colors for active state
                        : 'text-gray-700 hover:text-orange-600 hover:bg-orange-50 dark:text-[#F5F5F5] dark:hover:text-[#FF4C1B] dark:hover:bg-[#1A1A1A]/10' // Dark mode colors for default/hover
                    }`}
                    style={{ 
                  ...
                  >
                    <Icon size={16} className={`transition-all duration-300 ${isActive ? 'text-white dark:text-[#F5F5F5]' : 'text-gray-500 dark:text-[#999999]'}`} /> {/* Dark mode colors for icon */}
                    <span className="font-medium">{item.label}</span>
                    
                    {/* Active indicator */}
                    {isActive && (
                      <div className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 w-2 h-2 bg-white rounded-full dark:bg-[#FF4C1B] animate-pulse"></div> // Dark mode color for indicator
                    )}
                    
                    {/* Hover effect */}
                    <div className="absolute inset-0 bg-gradient-to-r from-orange-500 to-orange-600 rounded-full opacity-0 group-hover:opacity-10 transition-opacity duration-300 dark:from-[#FF4C1B]/10 dark:to-[#FF3C00]/10"></div> {/* Dark mode colors for hover effect */}
                  </button>
                );
              })}
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                onClick={toggleTheme}
                className="relative w-10 h-10 bg-white/10 backdrop-blur-md rounded-full flex items-center justify-center hover:bg-orange-500/20 transition-all duration-300 dark:bg-[#1A1A1A]/50 dark:hover:bg-[#FF4C1B]/20" // Dark mode colors for theme button
        ...
        <div className={`lg:hidden transition-all duration-300 ${
          ...
          <div className="bg-white/10 backdrop-blur-xl border-t border-white/20 px-6 py-4 dark:bg-[#0D0D0D] dark:bg-opacity-95 dark:border-white/5"> {/* Dark mode colors for mobile menu overlay */}
            <div className="space-y-3">
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-all duration-300 ${
                      isActive 
                        ? 'bg-gradient-to-r from-orange-500 to-orange-600 text-white shadow-lg dark:from-[#FF4C1B] dark:to-[#FF3C00] dark:text-[#F5F5F5]' // Dark mode colors for active state
                        : 'text-gray-700 hover:bg-orange-50 dark:text-[#F5F5F5] dark:hover:bg-[#1A1A1A]/10' // Dark mode colors for default/hover
                    }`}
                    style={{ 
                      animationDelay: `${index * 0.1}s`,
                  ...
                  >
                    <Icon size={20} className={isActive ? 'text-white dark:text-[#F5F5F5]' : 'text-gray-500 dark:text-[#999999]'} /> {/* Dark mode colors for icon */}
                    <span className="font-medium">{item.label}</span>
                  </button>
                );
              })}
              ...
              {/* Mobile Social Links */}
              <div className="flex justify-center space-x-4 pt-4 border-t border-white/20 dark:border-white/5"> {/* Dark mode colors for mobile social links divider */}
                {socialLinks.map((social, index) => {
                  const Icon = social.icon;
                  return (
                    <a
      ...
      <div className="fixed top-0 left-0 w-full h-1 bg-transparent z-50">
        <div 
          className="h-full bg-gradient-to-r from-orange-500 to-orange-600 transition-all duration-300 ease-out dark:from-[#FF4C1B] dark:to-[#FF3C00]" // Dark mode colors for scroll bar
          style={{ 
